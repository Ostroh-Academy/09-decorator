...
## UML-діаграма
![image](https://github.com/Ostroh-Academy/09-decorator-IcticStep/assets/59373161/efc00778-6092-42cd-ae98-6b3955949036)

## Короткий опис коду
Цей код використовує паттерн декоратор для динамічного додавання додаткової функціональності до базового класу ноутбука. Кожен декоратор додає певне покращення, таке як захист, більш потужний акумулятор або шифрування, не змінюючи базовий клас.

Інтерфейс `ILaptop` визначає базовий контракт для всіх ноутбуків. Клас BasicLaptop реалізує цей інтерфейс і представляє базовий ноутбук зі звичайними функціями та стандартною ціною.

Абстрактний клас `LaptopDecorator` також реалізує ILaptop. Цей клас представляє собою базовий декоратор для всіх покращень. Він містить посилання на об'єкт типу `ILaptop`, що дозволяє організувати ланцюжок декораторів.

Кожен конкретний декоратор, такий як `LaptopAdvanceProtectionDecorator`, `LaptopBiggerBatteryDecorator`, `LaptopSatelliteConnectionDecorator` та `LaptopSecretArmyEncryptionDecorator`, розширює клас `LaptopDecorator` додаючи свою унікальну функціональність, яка поєднується разом з функціональністю базового ноутбука, а також збільшує кінцеву вартість ноутбука.

Таким чином, можна легко розширювати існуючий функціонал ноутбука, створюючи нові класи замість редагування наявних, виконуючи Open-Closed-принцип з приципів SOLID.
